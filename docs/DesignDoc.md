...
**Status:** Research/planning only — nothing in this document has been implemented. This is a design-reference pass for the currently under-specified back half of the 14-step roadmap (Steps 9–14 in CLAUDE.md only had one-line bullets; Steps 1–8 already have full technical specs in CLAUDE.md itself). Treat this doc as a staging area: recommendations here get promoted into CLAUDE.md as a locked spec addendum once the Director signs off on the open questions at the bottom, at which point they become binding the same way Steps 1–8 already are.
