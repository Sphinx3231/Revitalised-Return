...
# Step 6: hit-flash shader (CLAUDE.md 6.3). Applied to MeshPlaceholder below;
# see assets/shaders/hit_flash.gdshader for the shader itself.
const HIT_FLASH_SHADER: Shader = preload("res://assets/shaders/hit_flash.gdshader")
const HIT_FLASH_DURATION: float = 0.08  # Total flash-in + decay window, CLAUDE.md 6.3.
...
# Step 6: CameraTrauma now lives directly on the CamPivot node (see
# scripts/player/camera_trauma.gd); cast rather than a second node lookup.
@onready var _camera_trauma: CameraTrauma = _cam_pivot as CameraTrauma
...
func _ready() -> void:
	...
	# Player isn't a Combatant (Step 5 sign-off) and nothing currently emits
	# entity_damaged with target_node==Player -- no enemy exists yet (Step 7).
	# The hook is wired now regardless, per this step's task list, so it's
	# real and testable rather than deferred.
...
func _physics_process(delta: float) -> void:
	...
	# at Engine.time_scale=0.0, so without this guard _dodge_tick would keep
	# silently advancing during a hit-stop freeze. See autoload/hit_stop.gd.
