...
## Current status
Steps 1-3 complete (project init; core FSM/EventBus autoloads; InputMap + rolling action buffer), each with a clean QA pass and full task log under `docs/Tasks/`. Steps 9-14's design pass is also locked (see `docs/DesignDoc.md`). Step 3 deliberately deviated from this charter's literal text: `stance_prev`'s **keyboard** binding is `Tab`, not `E` — the text above still says `E`, but as written it double-binds keyboard `E` to both `stance_prev` and `interact`, a real conflict caught during Step 3's research. Gamepad `stance_prev` (LT) is unaffected. Next action: Director opens the Step 4 task brief (`docs/Tasks/<date>-step-4-kinematics-dodge.md`) for **Step 4: 3D Kinematics, Movement Physics & Dodge Roll** and routes it through the pipeline.
