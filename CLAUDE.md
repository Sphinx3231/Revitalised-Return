...
This project is independent — it has no relation to any other project in sibling directories (e.g. NinjaGame). It lives in its own repo at `https://github.com/Sphinx3231/Revitalised-Return.git`, built in **Godot 4 (GDScript)**.
...
*   **Juice Engine:**
    ...
    *   **VFX:** White flash shader (`flash_strength`), `GPUParticles` sparks, `Line2D` slash trails.
...
4.  **Kinematics & Dodge:** `CharacterBody` physics, `lerp()` velocity, 0.2s–0.35s dodge i-frames.
5.  **Stances & Hitboxes:** Custom `Resource` stances + `Area` hit/hurtbox sweeps & parry timing.
6.  **Juice Engine:** Hit-stop, camera trauma, hit flashes, particle sparks, slash arcs.
7.  **AI Architecture:** 90° `RayCast` FOV, sound radiuses, FSM behavior nodes.
8.  **Boss Mechanics:** Multi-phase HP/posture thresholds, arena locks, camera tracking.
9.  **World Greybox:** Block out 5 regions with `TileMap`/`CSG` primitives & pacing checks.
...
├── assets/            (Textures, Sprites, Audio, Shaders)
...
2.  **Research Agent** (`Explore` for quick lookups, `general-purpose` with WebSearch/WebFetch for anything needing Godot docs or external references)
    *   Investigates existing systems it touches, relevant Godot 4 APIs/docs (`CharacterBody2D`, `Area2D`, `RayCast2D`, `AnimationTree`, `ResourceSaver`, etc.), how similar mechanics are usually implemented in Godot, and constraints (physics tick rate, signal ordering, node lifecycle).
