...
## Current status
Steps 1-5 complete (project init; core FSM/EventBus autoloads; InputMap + rolling action buffer; 3D kinematics/dodge; stance engine + hitbox/hurtbox combat resolution), each with a clean QA pass and full task log under `docs/Tasks/`. Steps 9-14's design pass is also locked (see `docs/DesignDoc.md`). Standing amendments to this charter's literal text, all logged as Director rulings in their respective task files rather than silently changed:
...
- **Step 5:** Section 5.2's "blocking" state has no InputMap action behind it — there is no `block` action in Section 3.1's list. `is_blocking` on `Combatant` is pure exposed state (public setter) with no input wired to it yet; something later needs to define how a player actually enters a blocking state (possibly stance-specific).

Known carried-forward debt from Step 5 (not blocking, but must be addressed when the relevant step starts): (1) `EventBus.posture_broken` is not edge-triggered — it re-fires on every hit once posture is already <=0; (2) every `Combatant` independently consumes the shared global `InputBuffer`'s `"parry"` action — harmless today (no enemy exists, `Player` isn't a `Combatant`), but **Step 7's enemy-AI task intake must address this** before extending `combatant.gd`, or every enemy will react to the player's own parry key-press.

Real playable content now exists: `scenes/player/player.tscn` (camera-relative movement/dodge, Step 4) plus a full combat resolution system (`scripts/combat/combatant.gd`, Step 5) with 4 data-driven stances (`resources/stances/*.tres`), hitbox/hurtbox collision via real `AnimationPlayer` method-call tracks, and parry/block/hit math. Stamina/Health/Posture/Armor are all still stubs pending Step 11/14's real stat/save system. Next action: Director opens the Step 6 task brief (`docs/Tasks/<date>-step-6-juice-engine.md`) for **Step 6: 3D "Juice" Engine & Impact Feedback** and routes it through the pipeline.
